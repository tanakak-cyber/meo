        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('masters.admins.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-4">
                            <label for="password" class="block text-sm font-medium text-gray-700 mb-2">
                                ログインパスワード <span class="text-red-500">*</span>
                            </label>
                            ...
                            <p class="mt-1 text-sm text-gray-500">管理者がログインする際に使用するパスワードです</p>
                            @error('password')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                権限設定 <span class="text-red-500">*</span>
                            </label>
                            <p class="mb-2 text-sm text-gray-500">表示する機能を選択してください</p>
                            <div class="space-y-2">
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="dashboard" {{ in_array('dashboard', old('permissions', [])) ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    <span class="ml-2 text-sm text-gray-700">ダッシュボード</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="shops.index" id="permission_shops_index" {{ in_array('shops.index', old('permissions', [])) ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    <span class="ml-2 text-sm text-gray-700">顧客一覧</span>
                                </label>
                                <div id="customer_scope_section" class="ml-8 mb-2 p-3 bg-gray-50 rounded-md" style="display: {{ in_array('shops.index', old('permissions', [])) ? 'block' : 'none' }};">
                                    <label class="block text-sm font-medium text-gray-700 mb-2">顧客閲覧範囲</label>
                                    <div class="space-y-2">
                                        <label class="flex items-center">
                                            <input type="radio" name="customer_scope" value="own" {{ old('customer_scope', 'own') === 'own' ? 'checked' : '' }}
                                                class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                            <span class="ml-2 text-sm text-gray-700">自分の顧客のみ</span>
                                        </label>
                                        <label class="flex items-center">
                                            <input type="radio" name="customer_scope" value="all" {{ old('customer_scope', 'own') === 'all' ? 'checked' : '' }}
                                                class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="shops.schedule" {{ in_array('shops.schedule', old('permissions', [])) ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    <span class="ml-2 text-sm text-gray-700">スケジュール</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="reviews.index" {{ in_array('reviews.index', old('permissions', [])) ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    <span class="ml-2 text-sm text-gray-700">口コミ管理</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="reports.index" {{ in_array('reports.index', old('permissions', [])) ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    <span class="ml-2 text-sm text-gray-700">月次レポート</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="gbp-insights.import" {{ in_array('gbp-insights.import', old('permissions', [])) ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    <span class="ml-2 text-sm text-gray-700">CSVインポート</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="report-email-settings.index" {{ in_array('report-email-settings.index', old('permissions', [])) ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    <span class="ml-2 text-sm text-gray-700">メール設定</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="masters.index" {{ in_array('masters.index', old('permissions', [])) ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    <span class="ml-2 text-sm text-gray-700">マスタ管理</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="permissions[]" value="competitor-analysis" {{ in_array('competitor-analysis', old('permissions', [])) ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
